    ...
    <DialogPrimitive.Root open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogPrimitive.Portal>
        ...
        {/*
          ...
          primitives elsewhere), which previously caused the dialog to
          render unpositioned (visually offset toward the bottom-right)
          for the animation's first frames before snapping into place.
        */}
        <DialogPrimitive.Content
          className={`fixed ${positionClass} z-[var(--z-modal)] focus:outline-none`}
        >
          <div
            className={`relative w-[92vw] ${widthClass} rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] shadow-2xl animate-scale-in max-h-[85vh] flex flex-col`}
          >
            {hideHeader ? (
              <>
